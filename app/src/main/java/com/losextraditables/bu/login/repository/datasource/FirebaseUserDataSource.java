...
public class FirebaseUserDataSource implements UserDatasource {
    ...
    public Observable<Void> createUser(final String username, final String password) {
        return Observable.create(new Observable.OnSubscribe<Void>() {
            ...
            public void call(final Subscriber<? super Void> subscriber) {
                getFirebaseConnection().createUser(username, password, new Firebase.ValueResultHandler<Map<String, Object>>() {
                    @Override
    ...
    public Observable<Void> login(final String username, final String password) {
        return Observable.create(new Observable.OnSubscribe<Void>() {
            @Override
            public void call(final Subscriber<? super Void> subscriber) {
                getFirebaseConnection().authWithPassword(username, password, new Firebase.AuthResultHandler() {
                    @Override
